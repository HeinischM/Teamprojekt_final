...
def define_model ():
	inputs = layers.Input(shape=(28, 28, 1), name='dae_input')
	...
	deconv_block2 = layers.ZeroPadding2D(padding = 3)(deconv_block2)

	merge = layers.Concatenate()([deconv_block2, conv_block1])
	...
	final_deconv = layers.Conv2DTranspose(filters=1, kernel_size=3, padding='same')(deconv_block3)
	outputs = layers.Activation('sigmoid')(final_deconv)

	model = Model(inputs, outputs)
	model.compile(loss='mean_squared_error', optimizer='adam')
	return model
# Creating Autoencoder for FashionMNIST, optional saving the trained Autoencoder
